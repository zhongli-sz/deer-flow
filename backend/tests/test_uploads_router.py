...
def _gateway_request_stub() -> MagicMock:
    """Minimal ``Request`` for calling upload handlers directly (tenancy off in tests)."""
    return MagicMock()
...
def test_upload_files_writes_thread_storage_and_skips_local_sandbox_sync(tmp_path):
    ...
    with (
        ...
        result = asyncio.run(uploads.upload_files("thread-local", _gateway_request_stub(), files=[file]))
...
def test_upload_files_skips_acquire_when_thread_data_is_mounted(tmp_path):
    ...
    with (
        ...
        file = UploadFile(filename="notes.txt", file=BytesIO(b"hello uploads"))
        result = asyncio.run(uploads.upload_files("thread-mounted", _gateway_request_stub(), files=[file]))
...
def test_upload_files_syncs_non_local_sandbox_and_marks_markdown_file(tmp_path):
    ...
    with (
        ...
        file = UploadFile(filename="report.pdf", file=BytesIO(b"pdf-bytes"))
        result = asyncio.run(uploads.upload_files("thread-aio", _gateway_request_stub(), files=[file]))
...
def test_delete_uploaded_file_removes_generated_markdown_companion(tmp_path):
    ...
    with patch.object(uploads, "get_uploads_dir", return_value=thread_uploads_dir):
        result = asyncio.run(uploads.delete_uploaded_file("thread-aio", "report.pdf", _gateway_request_stub()))
