...
_SAFE_TENANT_ID_RE = _SAFE_THREAD_ID_RE
...
class Paths:
    ...
    Directory layout (host side):
        ...
        │       └── memory.json
        └── threads/{thread_id}/... or tenants/{tenant_id}/threads/{thread_id}/...
    ...
    def thread_dir(self, thread_id: str, *, tenant_id: str | None = None) -> Path:
        """
        Host path for a thread's data.

        Layout:
            - Default: `{base_dir}/threads/{thread_id}/`
            - Tenant-scoped: `{base_dir}/tenants/{tenant_id}/threads/{thread_id}/`
        ...
        if tenant_id is None:
            return self.base_dir / "threads" / safe_thread_id
        return self.base_dir / "tenants" / _validate_tenant_id(tenant_id) / "threads" / safe_thread_id

    def sandbox_work_dir(self, thread_id: str, *, tenant_id: str | None = None) -> Path:
        """
        Host path for the agent's workspace directory.
        Host: `{base_dir}/threads/{thread_id}/user-data/workspace/`
        Sandbox: `/mnt/user-data/workspace/`
        """
        return self.thread_dir(thread_id, tenant_id=tenant_id) / "user-data" / "workspace"

    def sandbox_uploads_dir(self, thread_id: str, *, tenant_id: str | None = None) -> Path:
        """
        Host path for user-uploaded files.
        Host: `{base_dir}/threads/{thread_id}/user-data/uploads/`
        Sandbox: `/mnt/user-data/uploads/`
        """
        return self.thread_dir(thread_id, tenant_id=tenant_id) / "user-data" / "uploads"

    def sandbox_outputs_dir(self, thread_id: str, *, tenant_id: str | None = None) -> Path:
        """
        Host path for agent-generated artifacts.
        Host: `{base_dir}/threads/{thread_id}/user-data/outputs/`
        Sandbox: `/mnt/user-data/outputs/`
        """
        return self.thread_dir(thread_id, tenant_id=tenant_id) / "user-data" / "outputs"

    def acp_workspace_dir(self, thread_id: str, *, tenant_id: str | None = None) -> Path:
        """
        ...
        """
        return self.thread_dir(thread_id, tenant_id=tenant_id) / "acp-workspace"

    def sandbox_user_data_dir(self, thread_id: str, *, tenant_id: str | None = None) -> Path:
        """
        Host path for the user-data root.
        Host: `{base_dir}/threads/{thread_id}/user-data/`
        Sandbox: `/mnt/user-data/`
        """
        return self.thread_dir(thread_id, tenant_id=tenant_id) / "user-data"

    def host_thread_dir(self, thread_id: str, *, tenant_id: str | None = None) -> str:
        """Host path for a thread directory, preserving Windows path syntax."""
        safe_thread_id = _validate_thread_id(thread_id)
        if tenant_id is None:
            return _join_host_path(self._host_base_dir_str(), "threads", safe_thread_id)
        return _join_host_path(self._host_base_dir_str(), "tenants", _validate_tenant_id(tenant_id), "threads", safe_thread_id)

    def host_sandbox_user_data_dir(self, thread_id: str, *, tenant_id: str | None = None) -> str:
        """Host path for a thread's user-data root."""
        return _join_host_path(self.host_thread_dir(thread_id, tenant_id=tenant_id), "user-data")

    def host_sandbox_work_dir(self, thread_id: str, *, tenant_id: str | None = None) -> str:
        """Host path for the workspace mount source."""
        return _join_host_path(self.host_sandbox_user_data_dir(thread_id, tenant_id=tenant_id), "workspace")

    def host_sandbox_uploads_dir(self, thread_id: str, *, tenant_id: str | None = None) -> str:
        """Host path for the uploads mount source."""
        return _join_host_path(self.host_sandbox_user_data_dir(thread_id, tenant_id=tenant_id), "uploads")

    def host_sandbox_outputs_dir(self, thread_id: str, *, tenant_id: str | None = None) -> str:
        """Host path for the outputs mount source."""
        return _join_host_path(self.host_sandbox_user_data_dir(thread_id, tenant_id=tenant_id), "outputs")

    def host_acp_workspace_dir(self, thread_id: str, *, tenant_id: str | None = None) -> str:
        """Host path for the ACP workspace mount source."""
        return _join_host_path(self.host_thread_dir(thread_id, tenant_id=tenant_id), "acp-workspace")

    def ensure_thread_dirs(self, thread_id: str, *, tenant_id: str | None = None) -> None:
        ...
        """
        for d in [
            self.sandbox_work_dir(thread_id, tenant_id=tenant_id),
            self.sandbox_uploads_dir(thread_id, tenant_id=tenant_id),
            self.sandbox_outputs_dir(thread_id, tenant_id=tenant_id),
            self.acp_workspace_dir(thread_id, tenant_id=tenant_id),
        ]:
            d.mkdir(parents=True, exist_ok=True)
            d.chmod(0o777)

    def delete_thread_dir(self, thread_id: str, *, tenant_id: str | None = None) -> None:
        """Delete all persisted data for a thread.

        The operation is idempotent: missing thread directories are ignored.
        """
        thread_dir = self.thread_dir(thread_id, tenant_id=tenant_id)
        if thread_dir.exists():
            shutil.rmtree(thread_dir)

    def resolve_virtual_path(self, thread_id: str, virtual_path: str, *, tenant_id: str | None = None) -> Path:
        ...
        base = self.sandbox_user_data_dir(thread_id, tenant_id=tenant_id).resolve()
